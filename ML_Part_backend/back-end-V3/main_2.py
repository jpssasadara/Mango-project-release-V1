...
import ML_model_Testing_multi_threding_Env_2
...
@app.route('/count-all-rows')
def getCountAllRows():
        try:
                conn = mysql.connect()
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.execute("SELECT COUNT(*) FROM mango_data ")
                rows = cursor.fetchone()
                resp = jsonify(rows)
                resp.status_code = 200
                return resp
...
@app.route('/count-process-rows')
def getCountProcessRows():
        try:
                conn = mysql.connect()
                cursor = conn.cursor(pymysql.cursors.DictCursor)
                cursor.execute("SELECT COUNT(*)AS total" +
                               " FROM mango_data"+
                               " WHERE process_id = (select max(process_id) from process_data)")
                rows = cursor.fetchone()
                resp = jsonify(rows)
                resp.status_code = 200
                return resp
...
def getResult():
        try:
                resp = ML_model_Testing_multi_threding_Env_2.get_Grade()
                return resp
        except Exception as e:
                return e
...
def getResultSerial():
        try:
                resp = ML_model_Testing_multi_threding_Env_2.get_Grade_serial_execution()
                return jsonify(resp)
        except Exception as e:
                return e
##############################################################
...
@app.route('/create_mango/<int:id>/<string:grade>', methods=['POST'])
def create_mango(id,grade):
	try:
		conn = mysql.connect() 
		cursor = conn.cursor()
		cursor.execute("INSERT INTO mango_data(grade, process_id) VALUES(%s, %s)", (grade,id,))
		conn.commit()
